...
use App\Models\Master\Base\DeliveryFeeModel;
use Illuminate\Database\Eloquent\Builder;
use App\Modules\Order\Base\SearchDeliveryFeesInterface;

class SearchDeliveryFees implements SearchDeliveryFeesInterface
{
    /**
     * デフォルトの検索条件
     */
    ...
    public function setConditions($query, $conditions): Builder
    {
        ...
        // 配送別送料マスタID
        if(isset($conditions['m_delivery_fee_id'])){
            $query->where('m_delivery_fee_id', $conditions['m_delivery_fee_id']);
        }
        ...
        // 倉庫マスタID m_warehouses_id
        if(isset($conditions['m_warehouses_id'])){
            $query->where('m_warehouses_id', $conditions['m_warehouses_id']);
        }

        // 配送方法マスタID m_delivery_types_id
        if(isset($conditions['m_delivery_types_id'])){
            $query->where('m_delivery_types_id', $conditions['m_delivery_types_id']);
        }

        // 都道府県ID m_prefecture_id
        if(isset($conditions['m_prefecture_id'])){
            $query->where('m_prefecture_id', $conditions['m_prefecture_id']);
        }
