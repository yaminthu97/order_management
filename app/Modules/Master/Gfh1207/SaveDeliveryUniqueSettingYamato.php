...
use App\Modules\Master\Base\SaveDeliveryUniqueSettingYamatoInterface;
use Illuminate\Database\Eloquent\Model;
use Illuminate\Support\Facades\DB;

class SaveDeliveryUniqueSettingYamato implements SaveDeliveryUniqueSettingYamatoInterface
{
    //DBへの保存コード
    public function execute(array $fillData, array $exFillData): Model
    {
        ...
        try {
            ...
            $new = DB::transaction(function () use ($fillData, $exFillData) {
                if (isset($fillData['m_delivery_unique_setting_yamato_id'])) {
                    $new = DeliveryUniqueSettingYamatoModel::findOrFail($fillData['m_delivery_unique_setting_yamato_id']);
                } else {
                    $new = new DeliveryUniqueSettingYamatoModel();
                }
