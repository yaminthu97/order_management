...
use App\Models\Ami\Base\AmiEcAttachmentItemModel;
use Illuminate\Database\Eloquent\Builder;
use App\Modules\Ami\Base\SearchAmiEcAttachmentInterface;

class SearchAmiEcAttachment implements SearchAmiEcAttachmentInterface
{
    /**
     * デフォルトの検索条件
     */
    ...
    protected $defaultSorts = [
        'm_ami_ec_attachment_item_id' => 'asc'
    ];

    public function execute(array $conditions=[], array $options=[])
    {
        $query = AmiEcAttachmentItemModel::query();
    ...
    public function setConditions($query, $conditions): Builder
    {
        ...
        // 付属品マスタID
        if(isset($conditions['m_ami_ec_attachment_item_id'])){
            $query->where('m_ami_ec_attachment_item_id', $conditions['m_ami_ec_attachment_item_id']);
        }
        ...
        if(isset($conditions['group_id'])){
            $query->where('group_id', $conditions['group_id']);
        }
