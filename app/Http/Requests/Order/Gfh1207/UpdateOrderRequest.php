...
use App\Http\Requests\Order\Base\UpdateOrderRequest as BaseUpdateOrderRequest;
...
class UpdateOrderRequest extends BaseUpdateOrderRequest
{
    /**
    ...
    public function rules(): array
    {
        ...
        $rules = [
            ...
            'register_destination.*.delivery_name'				=>	[ 'nullable' ],
            ...
            'register_destination.*.register_detail.*.cancel_timestamp'			=>	[ 'nullable',	'required_with:cancel_timestamp',],

            // 熨斗情報
            'register_destination.*.register_detail.*.t_order_dtl_noshi'			=>	[ 'nullable',	'array' ],
            'register_destination.*.register_detail.*.t_order_dtl_noshi.count'						=>	[ 'nullable',	'numeric' ],
            'register_destination.*.register_detail.*.t_order_dtl_noshi.noshi_detail_id'				=>	[ 'nullable',	'numeric' ],
            'register_destination.*.register_detail.*.t_order_dtl_noshi.m_noshi_naming_pattern_id'	=>	[ 'nullable',	'str_length_max_count:256' ],
            'register_destination.*.register_detail.*.t_order_dtl_noshi.attach_flg'					=>	[ 'nullable',	'in:0,1' ],
            ...
            // 受注明細SKU情報
            'register_destination.*.register_detail.*.register_detail_sku'		=>	[ 'nullable',	'array' ],
            ...
            'register_destination.*.register_detail.*.register_detail_sku.*.item_cd'				=>	[ 'nullable',	'str_length_max_count:100' ],
            'register_destination.*.register_detail.*.register_detail_sku.*.item_vol'				=>	[ 'nullable',	'numeric',	'min:1' ]
        ];

        // 追加バリデーション
        $orderHdr = new OrderHdrModel();
        $orderDestination = new OrderDestinationModel();
        $orderDtl = new OrderDtlModel();
        $orderDtlSku = new OrderDtlSkuModel();
		...
		//受注基本存在チェック
		$rules['t_order_hdr_id'][] = Rule::exists($orderHdr->getTable(), $orderHdr->getKeyName())->where(function($query) {
			$query->where('cancel_timestamp', 0);
		});
		//受注配送先存在チェック
        /*
		$rules['register_destination.*.t_order_destination_id'][] = Rule::exists($orderDestination->getTable(), $orderDestination->getKeyName())->where(function($query) use($orderHdr, $pKey){
			$query->where($orderHdr->getKeyName(), $pKey);
		});
		//受注明細存在チェック
		$rules['register_destination.*.t_order_destination_id'][] = Rule::exists($orderDtl->getTable(), $orderDtl->getKeyName())->where(function($query) use($orderHdr, $pKey){
			$query->where($orderHdr->getKeyName(), $pKey);
		});
		//受注明細SKU存在チェック
		$rules['register_destination.*.t_order_destination_id'][] = Rule::exists($orderDtlSku->getTable(), $orderDtlSku->getKeyName())->where(function($query) use($orderHdr, $pKey){
			$query->where($orderHdr->getKeyName(), $pKey);
		});
        */
		//EC注文ID重複チェック
		...
		{
			$rules['ec_order_num'][] = Rule::unique($orderHdr->getTable())
				->ignore($pKey, $orderHdr->getKeyName())
        ...
        if(!empty($requestData['app_register_flg']))
        {
			...
			if(isset($requestData['return_flg']) && $requestData['return_flg'] == 1){
				$rules['order_total_price'][] = 'max:0';
			}else{
				$rules['order_total_price'][] = 'min:0';
			}

			// 注文主電話番号
			$rules['order_tel1'] = ['required_without_all:order_tel2', 'str_length_max_count:20',];
			$rules['order_tel2'] = ['required_without_all:order_tel1', 'str_length_max_count:20',];
			// 注文主郵便番号
			$rules['order_postal'] = [ 'required',	'str_length_max_count:7',	'postal7only' ];

			// 送付先電話番号
			$rules['register_destination.*.destination_tel'] = [ 'required',	'str_length_max_count:20', ];
			// 送付先郵便番号
			$rules['register_destination.*.destination_postal'] = [ 'required',	'str_length_max_count:7',	'postal7only' ];
        }

		//返品の場合、数値は-1以下
		if(isset($requestData['return_flg']) && $requestData['return_flg'] == 1){
			$rules['register_destination.*.register_detail.*.order_sell_vol']=[ 'required',	'numeric',	'max:-1' ];
			$rules['register_destination.*.register_detail.*.register_detail_sku.*.item_vol']=[ 'required',	'numeric',	'max:-1' ];
		}
    ...
    public function prepareForValidation()
    {
        ...
        // セッションからリクエストデータを復元
        $sessionKeyBase = 'order_edit_data_';
        $sessionKeyId = 'data_key_id';
        $editRow = [];
        $params = $this->input('params');
        if (isset($params)) {
            // params を復元
            $base64decodeParams = base64_decode($params);
            $decodeParams = json_decode($base64decodeParams, true);

            if (isset($decodeParams[$sessionKeyId])) {
                // $decodeParams に $sessionKeyId が含まれている場合、セッションから取得
                $editRow = Session::get($sessionKeyBase . $decodeParams[$sessionKeyId]);
            }
        }

        // セッションデータとリクエストデータをマージし、リクエストデータを優先
        $mergedData = array_merge($editRow, $this->all());
