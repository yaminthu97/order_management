...
class Pdf417 {
	...
	var $SizeKind = self::SIZE_KIND_AUTO;

	/*! 出力行数指定: SizeKindが出力行数指定の場合＝(自動サイズでない・列数指定でない場合)有効 */
	var $CodeRows = 3;

	/*! 出力データカラム数指定: SizeKindが出力列数指定の場合＝(自動サイズでない・行数指定でない場合)有効 */
	var $CodeColumns = 3;
	 ...
	 */
	function draw ($code, $minLinePitch) 
	{
		global $SizeKind, $CodeRows, $CodeColumns, $ErrorLevel, $UseAutoErrorLevel, $AspectRatio;

		$this->setText_String($code);
		$this->paintCode();
		...
		$x = 0;
		$y = -1;
		$out_Renamed = $this->outBits;
		$cols = (int)(((($this->bitColumns - 1)) / 8) + 1);
		$yBits = (int)((count($this->outBits) /*from: this.outBits.length*/ / $cols));
		if ((((count($this->outBits) /*from: this.outBits.length*/ % $cols)) != 0))
		{
			$yBits++;
		}
		$yBits *= 3;
		$xBits = $this->bitColumns;
		
		if(isOK()) {
			$img = ImageCreate($xBits * $minLinePitch, $yBits * $minLinePitch);
		} else {
			$img = ImageCreate($xBits * $minLinePitch, $yBits * $minLinePitch + 9);
		}
		...
		if(!isOK()) {
			$w = $xBits * $minLinePitch;
			$h = $yBits * $minLinePitch;
			ImageFilledRectangle($img, $w,$h, $w, $h + 9, $white);
		}

		//imagesetthickness($img, $this->BarThick);
		
		for ($k = 0; $k < count($out_Renamed); $k++) 
		{
			if (((($k % $cols)) == 0))
			{
				$y++;
			...
			for ($j = 0; ($j < 8); ++$j) 
			{
				if (((($tmp & ((0x80 >> $j)))) != 0))
				{
					for ($xx = 0; ($xx < $minLinePitch); ++$xx) 
					{
						for ($yy = 0; ($yy < (3 * $minLinePitch)); ++$yy) 
						{
							$xxx = $x * $minLinePitch + $xx;
							$yyy = $y * ($minLinePitch * 3) + $yy;
							//imagefilledrectangle($img, $xxx, $yyy, $xxx + 1,  $yyy + 1, $black);
							imagesetpixel ($img, $xxx, $yyy , $black );
						}
					}
				}
				++$x;
				if (($x == $this->bitColumns))
					break;
			}
		}
		...
		if(!isOK()) {
			...
			ImageTTFText($img, 9, 0, 0, $h+9 ,$red, "./font/mplus-1p-black.ttf", "SAMPLE");
		}
	...
	private static $PDF417_USE_RAW_CODEWORDS;	// int
	private static $PDF417_INVERT_BITMAP;	// int
	private static $START_PATTERN;	// int
	private static $STOP_PATTERN;	// int
	private static $START_CODE_SIZE;	// int
	private static $STOP_SIZE;	// int
	private static $MOD;	// int
	private static $ALPHA;	// int
	private static $LOWER;	// int
	private static $MIXED;	// int
	private static $PUNCTUATION;	// int
	private static $ISBYTE;	// int
	private static $BYTESHIFT;	// int
	private static $PL;	// int
	private static $LL;	// int
	private static $AS;	// int
	private static $ML;	// int
	private static $AL;	// int
	private static $PS;	// int
	private static $PAL;	// int
	private static $SPACE;	// int
	private static $TEXT_MODE;	// int
	private static $BYTE_MODE_6;	// int
	private static $BYTE_MODE;	// int
	private static $NUMERIC_MODE;	// int
	private static $ABSOLUTE_MAX_TEXT_SIZE;	// int
	private static $MAX_DATA_CODEWORDS;	// int
	private static $MIXED_SET;	// String
	private static $PUNCTUATION_SET;	// String
	private static $CLUSTERS;	// int
	private static $ERROR_LEVEL0;	// int
	private static $ERROR_LEVEL1;	// int
	private static $ERROR_LEVEL2;	// int
	private static $ERROR_LEVEL3;	// int
	private static $ERROR_LEVEL4;	// int
	private static $ERROR_LEVEL5;	// int
	private static $ERROR_LEVEL6;	// int
	private static $ERROR_LEVEL7;	// int
	private static $ERROR_LEVEL8;	// int
	private $outBits;	// byte[]
	private $bitColumns;	// int
	private $codewords;	// int[]
	private $lenCodewords = 0;	// int
	private $text;	// byte[]
	private $options;	// int
	private $yHeight;	// float
	...
	public static function __staticinit() { // static class members
		self::$PDF417_USE_RAW_CODEWORDS = 64;
		self::$PDF417_INVERT_BITMAP = 128;
		self::$START_PATTERN = 0x1fea8;
		self::$STOP_PATTERN = 0x3fa29;
		self::$START_CODE_SIZE = 17;
		self::$STOP_SIZE = 18;
		self::$MOD = 929;
		self::$ALPHA = 0x10000;
		self::$LOWER = 0x20000;
		self::$MIXED = 0x40000;
		self::$PUNCTUATION = 0x80000;
		self::$ISBYTE = 0x100000;
		self::$BYTESHIFT = 913;
		self::$PL = 25;
		self::$LL = 27;
		self::$AS = 27;
	...
	//public static function constructor__Graphics2D ($g) // [Graphics2D g]
	//{
	//	$me = new self();
	//	$this->__init();
	//	$this->pG = $g;
	...
	//	return $me;
	//}
	//public static function constructor__String ($filepath) // [String filepath]
	//{
	//	$me = new self();
	//	$this->__init();
	//	$this->setDefaultParameters();
	...
	private function setText_aB ($text) // [byte[] text]
	{
		$this->text = $text;
	}
	private function setText_String ($s) // [String s]
	{
		$encoding_sting = mb_convert_encoding($s, $this->StringEncoding, "UTF-8, EUC-JP, JIS, SJIS, eucjp-win, sjis-win");
		
		$bytes = array();
		
		for($i = 0; $i < strlen($encoding_sting); $i++){
			$bytes[] = ord($encoding_sting[$i]);
		}
		$this->text = $bytes;
	}
	/*
	private hextobin($hexstr) 
	{ 
		$n = strlen($hexstr); 
		$sbin="";   
		$i=0; 
	...
	*/ 
	private function getOutBits () 
	{
		return $this->outBits;
	}
	private function getBitColumns () 
	{
		return $this->bitColumns;
	}
	private function getCodewords () 
	{
		return $this->codewords;
	}
	private function getLenCodewords () 
	{
		return $this->lenCodewords;
	}
	private function setLenCodewords ($lenCodewords) // [int lenCodewords]
	{
	...
	private function setOptions ($options) // [int options]
	{
		$this->options = $options;
	}
	private function getYHeight () 
	{
		return $this->yHeight;
	}
	private function setYHeight ($yHeight) // [float yHeight]
	{
		$this->yHeight = $yHeight;
	}
	private function checkSegmentType ($segment, $type) // [Segment segment, char type]
	{
		if (($segment == NULL))
			return  FALSE ;
		return ($segment->type == $type);
	}
	private function getSegmentLength ($segment) // [Segment segment]
	{
		if (($segment == NULL))
			return 0;
	...
	{
		$this->UseAutoErrorLevel =  TRUE ;
		$this->ErrorLevel = 2;
		$this->SizeKind = self::SIZE_KIND_AUTO /* could not resolve enum */;
		$this->options = 0;
		$this->outBits = NULL;
		$this->text = array();
		$this->yHeight = 3;
		$this->AspectRatio = doubleval(0.5);
	}
	private function outCodeword17 ($codeword) // [int codeword]
	{
		$bytePtr = (int)($this->bitPtr / 8);
		$bit = ($this->bitPtr - ($bytePtr * 8));
		
		// ADD PAO
		if(!array_key_exists($bytePtr, $this->outBits)) $this->outBits[$bytePtr] = 0;
		$this->outBits[$bytePtr++] |= 0x000000ff & ($codeword >> ((9 + $bit)));
		
		// ADD PAO
		if(!array_key_exists($bytePtr, $this->outBits)) $this->outBits[$bytePtr] = 0;
		$this->outBits[$bytePtr++] |= 0x000000ff & ($codeword >> ((1 + $bit)));
		$codeword <<= 8;
	...
	private function outCodeword18 ($codeword) // [int codeword]
	{
		$bytePtr = (int)($this->bitPtr / 8);
		$bit = ($this->bitPtr - ($bytePtr * 8));

		if(!array_key_exists($bytePtr, $this->outBits)) $this->outBits[$bytePtr] = 0;
		$this->outBits[$bytePtr++] |= 0x000000ff & ($codeword >> ((10 + $bit)));
	
		// ADD PAO
		if(!array_key_exists($bytePtr, $this->outBits)) $this->outBits[$bytePtr] = 0;
		$this->outBits[$bytePtr++] |= 0x000000ff & ($codeword >> ((2 + $bit)));
		...
		{
			if(!array_key_exists($bytePtr, $this->outBits)) $this->outBits[$bytePtr] = 0;
				$this->outBits[$bytePtr++] |= 0x80;
		}
		$this->bitPtr += 18;
	}
	private function outCodeword ($codeword) // [int codeword]
	{
		$this->outCodeword17($codeword);
	}
	private function outStopPattern () 
	{
		$this->outCodeword18(self::$STOP_PATTERN);
	}
	private function outStartPattern () 
	{
		$this->outCodeword17(self::$START_PATTERN);
	}
	private function outPaintCode () 
	{
		$codePtr = 0;
		$this->bitColumns = ((self::$START_CODE_SIZE * (($this->CodeColumns + 3))) + self::$STOP_SIZE);
		$lenBits = ((((int)((($this->bitColumns - 1)) / 8) + 1)) * $this->CodeRows);
		$this->outBits = array();
		
		for ($row = 0; ($row < $this->CodeRows); ++$row) 
		{
			$this->bitPtr = ((((int)(((($this->bitColumns - 1)) / 8) + 1)) * 8) * $row);
			$rowMod = ($row % 3);
			$len_cluster = count(self::$CLUSTERS) / 3;
			$base_idx = $len_cluster * $rowMod;
			$cluster = self::$CLUSTERS; //[$rowMod];
			$this->outStartPattern();
			$edge = 0;
			switch ($rowMod) {
				case 0:
					$edge = (int)((30 * ((int)($row / 3))) + (int)(((($this->CodeRows - 1)) / 3)));
					break;
				case 1:
					$edge = (int)(((30 * ((int)($row / 3))) + ($this->ErrorLevel * 3)) + (((($this->CodeRows - 1)) % 3)));
					break;
				default:
					$edge = (int)(((30 * ((int)($row / 3))) + $this->CodeColumns) - 1);
					break;
			}
			$this->outCodeword($cluster[$base_idx + $edge]);
			for ($column = 0; ($column < $this->CodeColumns); ++$column) 
			{
				$this->outCodeword($cluster[$base_idx + $this->codewords[$codePtr++]]);
			}
			switch ($rowMod) {
				case 0:
					$edge = (int)(((30 * ((int)($row / 3))) + $this->CodeColumns) - 1);
		...
		if (((($this->options & self::$PDF417_INVERT_BITMAP)) != 0))
		{
			for ($k = 0; ($k < count($this->outBits) /*from: outBits.length*/); ++$k) 
				$this->outBits[$k] ^= 0xff;
		}
	}
	private function calculateErrorCorrection ($dest) // [int dest]
	{
		...
		//$A = self::$ERROR_LEVEL; //[$this->ErrorLevel];


		$Alength = (2 << $this->ErrorLevel);
		for ($k = 0; ($k < $Alength); ++$k) 
			$this->codewords[($dest + $k)] = 0;
		$lastE = ($Alength - 1);
		for ($k = 0; ($k < $this->lenCodewords); ++$k) 
		{
			$t1 = ($this->codewords[$k] + $this->codewords[$dest]);
			for ($e = 0; ($e <= $lastE); ++$e) 
			{
				$t2 = ((($t1 * $A[($lastE - $e)])) % self::$MOD);
				$t3 = (self::$MOD - $t2);
				$this->codewords[($dest + $e)] = ((((( (($e == $lastE)) ? 0 : $this->codewords[(($dest + $e) + 1)] )) + $t3)) % self::$MOD);
			}
		}
		for ($k = 0; ($k < $Alength); ++$k) 
			$this->codewords[($dest + $k)] = (((self::$MOD - $this->codewords[($dest + $k)])) % self::$MOD);
	}
	private function getTextTypeAndValue ($maxLength, $idx) // [int maxLength, int idx]
	{
		
		if (($idx >= $maxLength))
			return 0;
		$c = (($this->text[$idx] & 0xff));
		if ((($c >= ord('A')) && ($c <= ord('Z'))))
			return ((self::$ALPHA + $c - ord('A')));
		if ((($c >= ord('a')) && ($c <= ord('z'))))
			return ((self::$LOWER + $c - ord('a')));
		if (($c == ord(' ')))
			return ((((self::$ALPHA + self::$LOWER) + self::$MIXED) + self::$SPACE));
		$ms = strpos(self::$MIXED_SET, $c);
		$ps = strpos(self::$PUNCTUATION_SET, $c);
		if ((($ms == false) && ($ps == false)))
			return ((self::$ISBYTE + $c));
		if (($ms == $ps))
			return (((self::$MIXED + self::$PUNCTUATION) + $ms));
		if (($ms >= 0))
			return ((self::$MIXED + $ms));
		return ((self::$PUNCTUATION + $ps));
	}
	private function textCompaction ($start, $length) // [int start, int length]
	{
		$dest = array();
		$mode = self::$ALPHA;
		$ptr = 0;
		...
		{
			$v = $this->getTextTypeAndValue($length, $k);
			if (((($v & $mode)) != 0))
			{
				$dest[$ptr++] = ($v & 0xff);
				continue;
			...
			if (((($v & self::$ISBYTE)) != 0))
			{
				if (((($ptr & 1)) != 0))
				{
					$dest[$ptr++] = ( (((($mode & self::$PUNCTUATION)) != 0)) ? self::$PAL : self::$PS );
					$mode = ( (((($mode & self::$PUNCTUATION)) != 0)) ? self::$ALPHA : $mode );
				}
				$dest[$ptr++] = self::$BYTESHIFT;
				$dest[$ptr++] = ($v & 0xff);
				$fullBytes += 2;
				continue;
			}
			switch ($mode) {
				case self::$ALPHA:
					if (((($v & self::$LOWER)) != 0))
					{
						...
						if (((($v & self::$MIXED)) != 0))
						{
							...
							if ((((($this->getTextTypeAndValue($length, ($k + 1)) & $this->getTextTypeAndValue($length, ($k + 2))) & self::$PUNCTUATION)) != 0))
							{
								$dest[$ptr++] = self::$ML;
								$dest[$ptr++] = self::$PL;
								$dest[$ptr++] = ($v & 0xff);
								$mode = self::$PUNCTUATION;
							}
							else
							{
								$dest[$ptr++] = self::$PS;
				...
				case self::$LOWER:
					if (((($v & self::$ALPHA)) != 0))
					{
						if ((((($this->getTextTypeAndValue($length, ($k + 1)) & $this->getTextTypeAndValue($length, ($k + 2))) & self::$ALPHA)) != 0))
						{
							$dest[$ptr++] = self::$ML;
							$dest[$ptr++] = self::$AL;
							$mode = self::$ALPHA;
		...
		if ((($size + $this->cwPtr) > self::$MAX_DATA_CODEWORDS))
		{
	...
	private function basicNumberCompaction ($start, $length) // [int start, int length]
	{
		$ret = $this->cwPtr;
		$retLast = ($length / 3);
		$ni = null;
		...
		{
			for ($k = $retLast; ($k >= 0); --$k) 
				$this->codewords[($ret + $k)] *= 10;
			$this->codewords[($ret + $retLast)] += ($this->text[$ni] . '0');
			for ($k = $retLast; ($k > 0); --$k) 
			{
				$this->codewords[(($ret + $k) - 1)] += ($this->codewords[($ret + $k)] / 900);
				$this->codewords[($ret + $k)] %= 900;
			}
		}
	}
	private function numberCompaction ($start, $length) // [int start, int length]
	{
		$full = ((($length / 44)) * 15);
		$size = ($length % 44);
		$k = null;
		...
		for ($k = $start; ($k < $length); $k += 44) 
		{
			...
			$this->basicNumberCompaction($k, $size);
		}
	}
	private function byteCompaction6 ($start) // [int start]
	{
		$length = 6;
		$ret = $this->cwPtr;
		$retLast = 4;
		...
		{
			for ($k = $retLast; ($k >= 0); --$k) 
				$this->codewords[($ret + $k)] *= 256;
			$this->codewords[($ret + $retLast)] += ($this->text[$ni] & 0xff);
			for ($k = $retLast; ($k > 0); --$k) 
			{
				$this->codewords[(($ret + $k) - 1)] += (int)($this->codewords[($ret + $k)] / 900);
				$this->codewords[($ret + $k)] %= 900;
			}
		}
	}
	private function byteCompaction ($start, $length) // [int start, int length]
	{
		$k = null;
		$j = null;
	...
	private function breakString () 
	{
		$textLength = count($this->text) /*from: text.length*/;
		$lastP = 0;
		$startN = 0;
		$nd = 0;
		$c = 0;
		...
		for ($k = 0; ($k < $this->segmentList->size()); ++$k) 
		{
			$v = $this->segmentList->get($k);
			$vp = $this->segmentList->get(($k - 1));
			$vn = $this->segmentList->get(($k + 1));
			if (($this->checkSegmentType($v, 'B') && ($this->getSegmentLength($v) == 1)))
			{
				if ((($this->checkSegmentType($vp, 'T') && $this->checkSegmentType($vn, 'T')) && (($this->getSegmentLength($vp) + $this->getSegmentLength($vn)) >= 3)))
				{
					$vp->end = $vn->end;
		...
		$v = false;
		if($this->segmentList->get(0)->type == 'T')
		{
			$v = true;
		}
		
		//if (((($this->segmentList->size() == 1) . (($v = $this->segmentList->get(0))::$type . 'T')) && ($this->getSegmentLength($v) >= 8)))
		if (($this->segmentList->size() == 1) . $v && ($this->getSegmentLength($v) >= 8))
		{
	...
	private function assemble () 
	{
		...
		for ($k = 0; ($k < $this->segmentList->size()); ++$k) 
		{
			...
			switch ($v->type) {
				case 'T':
					if (($k != 0))
						$this->codewords[$this->cwPtr++] = self::$TEXT_MODE;
					$this->textCompaction($v->start, $this->getSegmentLength($v));
					break;
				case 'N':
					$this->codewords[$this->cwPtr++] = self::$NUMERIC_MODE;
					$this->numberCompaction($v->start, $this->getSegmentLength($v));
					break;
				case 'B':
					$this->codewords[$this->cwPtr++] = ( (((($this->getSegmentLength($v) % 6)) != 0)) ? self::$BYTE_MODE : self::$BYTE_MODE_6 );
					$this->byteCompaction($v->start, $this->getSegmentLength($v));
					break;
			}
		}
	}
	private static function maxPossibleErrorLevel ($remain) // [int remain]
	{
		$level = 8;
		$size = 512;
		while (($level > 0)) 
	...
	private function dumpList () 
	{
		...
		{
			$v = $this->segmentList->get($k);
			$len = $this->getSegmentLength($v);
			$c = array();
			for ($j = 0; ($j < $len); ++$j) 
			{
				$c[$j] = (($this->text[($v->start + $j)] & 0xff));
				if (($c[$j] . '\r'))
					$c[$j] = '\n';
			}
			print ( (("" . $v->type) . new String($c)) . "\n");
		}
	}
	private function getMaxSquare () 
	{
		...
		return (self::$MAX_DATA_CODEWORDS + 2);
	}
	public function paintCode () 
	{
		$maxErr = null;
		$lenErr = null;
		$tot = null;
		$pad = null;
		if (((($this->options & self::$PDF417_USE_RAW_CODEWORDS)) != 0))
		{
			if (((($this->lenCodewords > self::$MAX_DATA_CODEWORDS) || ($this->lenCodewords < 1)) || ($this->lenCodewords != $this->codewords[0])))
			{
		...
		else
		{
			...
			if ((count($this->text) /*from: text.length*/ > self::$ABSOLUTE_MAX_TEXT_SIZE))
			{
				throw new IndexOutOfBoundsException("The text is too big.");
			}
		...
		$maxErr = Pdf417::maxPossibleErrorLevel(((self::$MAX_DATA_CODEWORDS + 2) - $this->lenCodewords));
		if (($this->UseAutoErrorLevel ==  true ))
		{
			if (($this->lenCodewords < 41))
				$this->ErrorLevel = 2;
			else
				if (($this->lenCodewords < 161))
					$this->ErrorLevel = 3;
				else
					if (($this->lenCodewords < 321))
						$this->ErrorLevel = 4;
					else
						$this->ErrorLevel = 5;
		}
		...
		$fixedColumn = (($this->SizeKind != self::SIZE_KIND_ROWS /* could not resolve enum */));
		$skipRowColAdjust =  FALSE ;
		$tot = ($this->lenCodewords + $lenErr);
		if (($this->SizeKind == self::SIZE_KIND_COLUMNS_AND_ROWS /* could not resolve enum */))
		{
			$tot = ($this->CodeColumns * $this->CodeRows);
			if (($tot > (self::$MAX_DATA_CODEWORDS + 2)))
			{
		...
		if ($this->SizeKind == self::SIZE_KIND_AUTO)
		{
			$c = null;
			$b = null;
			$fixedColumn =  TRUE ;
			if (($this->AspectRatio < doubleval(0.001)))
			{
				$this->AspectRatio = doubleval(0.001);
			}
			else
			{
				if (($this->AspectRatio > 1000))
				{
					$this->AspectRatio = 1000;
				}
			}
			$b = ((73 * $this->AspectRatio) - 4);
				$c = (((-$b + sqrt(($b * $b) + ((((4 * 17) * $this->AspectRatio) * (($this->lenCodewords + $lenErr))) * $this->yHeight)))) / (((2 * 17) * $this->AspectRatio)));
				$this->CodeColumns = (int)(($c + doubleval(0.5)));
			if (($this->CodeColumns < 1))
			{
				$this->CodeColumns = 1;
			}
		...
		if (!$skipRowColAdjust)
		{
			if ($fixedColumn)
			{
				$this->CodeRows = ((int)((($tot - 1)) / $this->CodeColumns) + 1);
				if (($this->CodeRows < 3))
					$this->CodeRows = 3;
				else
					if (($this->CodeRows > 90))
		...
		$this->ErrorLevel = Pdf417::maxPossibleErrorLevel(($tot - $this->lenCodewords));
		$lenErr = (2 << $this->ErrorLevel);
		$pad = (($tot - $lenErr) - $this->lenCodewords);
...
class Segment {
	...
	public $start;	// int
	public $end;	// int
	function __construct ($type, $start, $end) // [char type, int start, int end]
	{
		$this->type = $type;
		$this->start = $start;
		$this->end = $end;
...
class SegmentList {
	protected $list;	// ArrayList<Segment>
	function __constract() { // default class members
		//$this->list = new ArrayList();
	}
	public function add ($type, $start, $end) // [char type, int start, int end]
	{
		//$this->list.add(new Segment($type, $start, $end));
		
		if($this->list != null)
		{
			$this->list[count($this->list)] = new Segment($type, $start, $end);
		}
		else
		{
			$this->list[0] = new Segment($type, $start, $end);
		}
		//array_push($this->list, new Segment($type, $start, $end));

	}
	public function get ($idx) // [int idx]
	{
		if ((($idx < 0) || ($idx >= count($this->list))))
			return NULL;
		return $this->list[$idx];
	}
	public function remove ($idx) // [int idx]
	{
		if ((($idx < 0) || ($idx >= count($this->list))))
			return ;
...
Pdf417::__staticinit(); // initialize static vars for this class on load
?>
