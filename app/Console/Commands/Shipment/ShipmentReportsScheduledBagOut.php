...
class ShipmentReportsScheduledBagOut extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'command:ShipmentReportsScheduledBagOut {t_execute_batch_instruction_id : バッチ実行指示ID} {json? : JSON化した引数}';
    ...
    protected $batchName = '出荷予定日別手提げ枚数';
    ...
    // for report name
    protected $reportName = TemplateFileNameEnum::EXPXLSX_SHIPMENT_REPORTS_SCHEDULED_BAG->value;
                ...
                foreach ($group1 as $destination) {
                    ...
                    foreach ($destination['order_dtls'] as $orderDtl) {
                        foreach ($orderDtl['attachment_items'] as $attachmentItem) {
                            ...
                            $attachmentVol += $attachmentItem['attachment_vol'] ?? 0;
                        }
                    }
                }
