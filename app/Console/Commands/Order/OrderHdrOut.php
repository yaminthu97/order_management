...
class OrderHdrOut extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'command:OrderHdrOut {t_execute_batch_instruction_id : バッチ実行指示ID} {json? : JSON化した引数}';
    ...
    protected $batchName = '受注一覧表';

    // for report name
    protected $reportName = TemplateFileNameEnum::EXPXLSX_ORDER_HDR->value;
    ...
    private function getData($param)
    {
        ...
        foreach ($query as $hdr) {
            $resultItems[] = [
                ...
                '最終更新ユーザ名' => $hdr['update_operator']['m_operator_name'] ?? null,
                '最終更新タイムスタンプ' => \Carbon\Carbon::parse($hdr['update_timestamp'])->toDateTimeString()
            ];
        }
    ...
    */
    ...
    {
        $keys = [
                        '受注ID','進捗区分名','受注日時','ECサイト名','ECサイト注文ID','受注方法名','備考','社内メモ','キャンセル理由名','キャンセル備考','コメント確認区分','要注意顧客区分','住所確認区分',
                        '配達指定日確認区分','与信区分','入金区分','在庫引当区分','出荷指示区分','出荷確定区分','決済売上計上区分','売上ステータス反映区分','支払い方法名','入金日','入金済額','顧客ID',
                        '注文者氏名','注文者カナ','注文者法人名・団体名','注文者電話番号','注文者電話番号２','注文者メールアドレス','注文者メールアドレス２','注文者郵便番号','注文者都道府県','注文者市区町村',
                        '注文者番地','注文者建物名','請求先氏名','請求先カナ','請求先法人名・団体名','請求先電話番号','請求先者電話番号２','請求先メールアドレス','請求先メールアドレス２','請求先郵便番号',
                        '請求先都道府県','請求先市区町村','請求先番地','請求先建物名','割引金額','ストアクーポン利用額','モールクーポン利用額','クーポン利用額計','利用ポイント','請求金額','標準税率合計金額',
