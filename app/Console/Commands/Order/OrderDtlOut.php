...
class OrderDtlOut extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'command:OrderDtlOut {t_execute_batch_instruction_id : バッチ実行指示ID} {json? : JSON化した引数}';
    ...
    // for report name
    protected $reportName = TemplateFileNameEnum::EXPXLSX_ORDER_DTL->value;
    ...
    */
    private function getData($param)
    {
        ...
        $query = OrderHdrModel::with([
                       'orderDtls' => function ($query) {
                           $query->orderBy('order_dtl_seq');
                       },
                       'orderDtls.orderDtlNoshi',
                       'orderDtls.orderDtlNoshi.noshiDetail',
                       'orderDtls.orderDtlNoshi.noshiDetail.noshiFormat',
                       'orderDtls.orderDtlNoshi.noshiDetail.noshiNamingPattern',
                       'orderDtls.orderDtlAttachmentItem',
                       'orderDtls.orderDestinations' => function ($query) {
                           $query->orderBy('order_destination_seq');
                       },
                       'orderDtls.orderDestinations.updateOperator',
                       'orderDtls.orderDestinations.deliveryType',
                       'orderDtls.orderDestinations.deliHdr',
                       'orderDtls.orderDestinations.deliHdr.shippingLabels',
                       'paymentTypes',
                       'ecs',
                       'orderType',
                       'orderMemo',
        ...
        foreach ($finalData as $hdr) {
            foreach ($hdr['order_dtls'] as $dtl) {
                $companyNames = array_filter([
                    $dtl['order_dtl_noshi']['company_name1'] ?? null,
                    $dtl['order_dtl_noshi']['company_name2'] ?? null,
                    $dtl['order_dtl_noshi']['company_name3'] ?? null,
                    $dtl['order_dtl_noshi']['company_name4'] ?? null,
                    $dtl['order_dtl_noshi']['company_name5'] ?? null,
                ]);

                $formattedData = array_map(function ($item) {
                    return "{$item['attachment_item_cd']} x {$item['attachment_vol']}";
                }, $dtl['order_dtl_attachment_item']);

                $accessorie = implode('/', $formattedData);

                foreach ($dtl['order_destinations']['deli_hdr'] as $deli) {
                    $resultItems[] = [
                        ...
                        '送付先番号' => $dtl['order_destinations']['order_destination_seq'] ?? null,
                        '送付先氏名' => $dtl['order_destinations']['destination_name'] ?? null,
                        '送付先カナ' => $dtl['order_destinations']['destination_name_kana'] ?? null,
                        '送付先法人名・団体名' => $dtl['order_destinations']['destination_company_name'] ?? null,
                        '送付先郵便番号' => $dtl['order_destinations']['destination_postal'] ?? null,
                        '送付先都道府県' => $dtl['order_destinations']['destination_address1'] ?? null,
                        '送付先市区町村' => $dtl['order_destinations']['destination_address2'] ?? null,
                        '送付先番地' => $dtl['order_destinations']['destination_address3'] ?? null,
                        '送付先建物名' => $dtl['order_destinations']['destination_address4'] ?? null,
                        '送付先電話番号' => $dtl['order_destinations']['destination_tel'] ?? null,
                        '配送方法' => $dtl['order_destinations']['delivery_type']['m_delivery_type_name'] ?? null,
                        '配送希望日' => $dtl['order_destinations']['deli_hope_date'] ?? null,
                        '配送希望時間' => $dtl['order_destinations']['deli_hope_time_name'] ?? null,
                        '配送日' => $deli['deli_decision_date'] ?? null,
                        '送り状番号' => implode('/', array_column($deli['shipping_labels'], 'shipping_label_number')),
                        ...
                        '販売コード' => $dtl['sell_cd'] ?? null,
                        '販売名' => $dtl['sell_name'] ?? null,
                        '受注価格' => $dtl['order_sell_price'] ?? null,
                        '受注数' => $dtl['order_sell_vol'] ?? null,
                        '在庫引当(販売)' => $dtl['temp_reservation_flg'] != "" ? ReservationTypeEnum::from($dtl['temp_reservation_flg'])->label() : null,
                        '原価' => $dtl['order_cost'] ?? null,
                        '熨斗タイプ' => $dtl['order_dtl_noshi']['noshi_type'] ?? null,
                        '熨斗種類' => $dtl['order_dtl_noshi']['noshi_detail']['noshi_format']['noshi_format_name'] ?? null,
                        '名入れパターン' => $dtl['order_dtl_noshi']['noshi_detail']['noshi_naming_pattern']['pattern_name'] ?? null,
                        '表書き' => $dtl['order_dtl_noshi']['omotegaki'] ?? null,
                        '熨斗名前' => implode('/', $companyNames),
                        '付属品' => $accessorie ,
                        '最終更新ユーザ名' => $dtl['order_destinations']['update_operator']['m_operator_name'] ?? null,
                        '最終更新タイムスタンプ' => isset($dtl['order_destinations']['update_timestamp']) ? Carbon::parse($dtl['order_destinations']['update_timestamp'])->toDateTimeString() : null,
                    ];
                }
            }
        }
    ...
    */
    ...
    {
        $keys = [
                        '受注ID','進捗区分名','受注日時','ECサイト名','ECサイト注文ID','受注方法名','備考','社内メモ','キャンセル理由名','キャンセル備考','コメント確認区分'
                        ,'要注意顧客区分','住所確認区分','配達指定日確認区分','与信区分','入金区分','在庫引当区分','出荷指示区分','出荷確定区分','決済売上計上区分'
                        ,'売上ステータス反映区分','支払い方法名','入金日','入金済額','顧客ID','注文者氏名','注文者カナ','注文者法人名・団体名','注文者電話番号'
                        ,'注文者電話番号２','注文者メールアドレス','注文者メールアドレス２','注文者郵便番号','注文者都道府県','注文者市区町村','注文者番地','注文者建物名'
                        ,'請求先顧客ID','請求先氏名','請求先カナ','請求先法人名・団体名','請求先電話番号','請求先者電話番号２','請求先メールアドレス','請求先メールアドレス２'
                        ,'請求先郵便番号','請求先都道府県','請求先市区町村','請求先番地','請求先建物名','送付先番号','送付先氏名','送付先カナ','送付先法人名・団体名'
                        ,'送付先郵便番号','送付先都道府県','送付先市区町村','送付先番地','送付先建物名','送付先電話番号','配送方法','配送希望日','配送希望時間','配送日'
                        ,'送り状番号','受注商品金額','受注送料','受注手数料','受注包装料','受注消費税額','受注合計金額','割引金額','ストアクーポン利用額','モールクーポン利用額'
                        ,'クーポン利用額計','利用ポイント','請求金額','標準税率合計金額','軽減税率合計金額','標準税率消費税','軽減税率消費税','販売コード','販売名','受注価格'
                        ,'受注数','在庫引当(販売)','原価','熨斗タイプ','熨斗種類','名入れパターン','表書き','熨斗名前','付属品' ,'最終更新ユーザ名','最終更新タイムスタンプ'
                     ];
        $data = [];
