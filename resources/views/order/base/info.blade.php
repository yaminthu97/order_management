...
@section('content')
												...
												<tr>
													...
													<td>
														@if( $orderDest->total_deli_flg == 1 )
															同梱出荷
															@isset( $orderDest->total_temperature_zone_type )
																：{{ \App\Enums\ThreeTemperatureZoneTypeEnum::tryfrom( $orderDest->total_temperature_zone_type )?->label() }}
															@endif
														@else
															同梱しない
														@endif
														...
														<td class="nopadding">
															...
															<table class="table table-bordered c-tbl c-tbl--1490 nomargin">
																<tr>
																	...
																	<td class="c-box--600 nopadding" rowspan="2">
																		<div class="page_desc">
																		...
																		<div class="c-box-height--125 overflow-auto-y">
																			@isset( $orderDtl->orderDtlAttachmentItem )
																				@foreach( $orderDtl->orderDtlAttachmentItem as $orderItem )
																					@isset( $orderItem->amiAttachmentItem )
																						<div class="u-mb--xs">
																							<span class="font-757575">付属品コード：</span>{{ $orderItem->amiAttachmentItem->attachment_item_cd ?? '' }}
																							<span class="font-757575">&nbsp;&nbsp;/&nbsp;&nbsp;付属品名：</span>{{ $orderItem->amiAttachmentItem->attachment_item_name ?? '' }}
		...
		<div class="c-box--1600 u-mt--ms">
			...
			<div class="u-mt--xs">
				<div class="d-table">
					...
					@endif
					@if( $order->progress_type > \App\Enums\ProgressTypeEnum::Shipping->value && $order->progress_type < \App\Enums\ProgressTypeEnum::Returned->value ) -->
						<div class="d-table-cell c-box--100 u-pr--mm">
	...
	@push('css')<link rel="stylesheet" href="{{ esm_internal_asset('css/order/base/app.css') }}">@endpush
	@push('js')<script src="{{ esm_internal_asset('js/order/base/info.js') }}"></script>@endpush
