...
{{ csrf_field() }}
	<div>
		<table class="table table-bordered c-tbl c-tbl--full">
			...
			<tr>
				...
				<td colspan="2">
                    ...
                    <div class="radio-inline"><label><input type="radio" name="font_color" value="{{$tableIdValue1}}" @checked(old('font_color', $records['font_color'] ?? \App\Modules\Order\Gfh1207\Enums\FontColorEnum::BLACK->value) == $tableIdValue1)> {{$tableIdName1}}</label></div>
                    @endforeach
			...
			<tr>
				...
				<td>
					<label class="checkbox-inline"><input type="checkbox" class="length_flg" name="{{"cond{$i}_length_flg"}}" value="{{$records["cond{$i}_length_flg"] ?? '1'}}" @checked(old("cond{$i}_length_flg", $records["cond{$i}_length_flg"] ?? '') == '1')> 項目の文字数を条件とする</label>
					<input type="hidden"  name="{{"cond{$i}_length_flg"}}" id="{{"cond{$i}_length_flg"}}" value="{{old("cond{$i}_length_flg", $records["cond{$i}_length_flg"] ?? '')}}">
        ...
        <button class="btn btn-success btn-lg u-mt--sm" type="submit" name="submit" id="submit_register" value="edit">確認</button>
