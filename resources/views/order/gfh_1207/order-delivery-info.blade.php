                ...
                <tr>
					...
					<td>
                        @if($deliveryDetailSku->amiSku)
                            @foreach ($deliveryDetailSku->amiSku as $amiSku)
                                {{$amiSku->item_name}}
                            @endforeach
                        @else
                            <div class="error u-mt--xs">
                        ...
                        <td>
                            <div class="d-flex" style="align-items: center;">
                                ...
                                <select @class([
                                    ...
                                    'c-box--200',
                                    'ml-10',
                                    'error-txtfield' => $errors->has('three_temperature_zone_types.' . $shippingLabel->t_shipping_label_id)
                                ]) name="three_temperature_zone_types[{{$shippingLabel->t_shipping_label_id}}]" id="three_temperature_zone_type" >
                                    @foreach($threeTemperatureZoneTypes as $type)
                                        <option value="{{$type->value}}" @selected(old('three_temperature_zone_types.' . $shippingLabel->t_shipping_label_id) !== null? old('three_temperature_zone_types.' . $shippingLabel->t_shipping_label_id) == (string)$type->value : $shippingLabel->three_temperature_zone_type == $type)>{{$type->label()}}</option>
                                    @endforeach
                                </select>
                            </div>
