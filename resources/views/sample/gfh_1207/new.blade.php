    ...
    <form method="POST" action="" name="Form1" id="Form1">
        ...
        <div class="u-mt--ss">
            ...
            @if (isset($sample->m_cust_id) && !$sample->isDeleted())
                <input type="submit" name="submit_delete" id="submit_delete" class="btn btn-danger btn-lg u-mt--sm"
                    value="削除">
            @endif
            @if ( !$sample->isDeleted())
                <input type="submit" name="submit_register" id="submit_register"
                    class="btn btn-success btn-lg u-mt--sm" value="確認">
            @endif
        </div>

        {{-- <input type="hidden" name="previous_url" value="{{ $editRow['previous_url'] or '' }}">
        <input type="hidden" name="previous_subsys" value="{{ $editRow['previous_subsys'] or '' }}">
        <input type="hidden" name="previous_key" value="{{ $editRow['previous_key'] or '' }}">
        <input type="hidden" name="{{ config('define.session_key_id') }}"
            value="{{ $editRow[config('define.session_key_id')] or '' }}"> --}}
